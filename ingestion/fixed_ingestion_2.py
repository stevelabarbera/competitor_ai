...
from metadata_chunker import MetadataChunker  # Base chunker class
from chunk_filtering.quality_filter import QualityFilter      # Import new post-filter
        ...
        try:
            with open(path, "r", encoding=enc) as f:
                ...
                if isinstance(result, tuple):
                    print(f"🚨 Tuple detected in {path}: {result}")
                    result = result[0]
                return result.strip()
...
def read_pdf(path):
    try:
        with fitz.open(path) as doc:
            ...
            if isinstance(result, tuple):
                print(f"🚨 Tuple detected in PDF {path}: {result}")
                result = result[0]
            return result
...
'''
#Deprecated these due two issues with allowing non string data to return (e.g. tuples)
def read_txt(path):
    for enc in ['utf-8', 'latin-1', 'cp1252']:
        try:
            with open(path, "r", encoding=enc) as f:
...
def ingest_documents(
    ...
    filter_obj = None if skip_filter else QualityFilter()
